...
ASCSkillElectroSpark::ASCSkillElectroSpark()
{
	...
	static ConstructorHelpers::FObjectFinder<UMaterial> texture(TEXT("Material'/Game/MatroidVania/Materials/Decal_ElectroSpark.Decal_ElectroSpark'"));
...
void ASCSkillElectroSpark::BeginPlay()
{
	Super::BeginPlay();

	InteractiveBox->OnComponentBeginOverlap.AddDynamic(this, &ASCSkillElectroSpark::OnOverlapBegin);
	InteractiveBox->OnComponentEndOverlap.AddDynamic(this, &ASCSkillElectroSpark::OverlapEnd);
...
void ASCSkillElectroSpark::OnOverlapBegin(UPrimitiveComponent* OverlappedComponent, AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult & SweepResult)
{
	if (OtherActor)
	{
		if (Player)
		{
			...
			if (!Player->bHasBMagicElectroSpark && !Player->bIsUsingMist)
			{
...
void ASCSkillElectroSpark::OverlapEnd(UPrimitiveComponent* OverlappedComponent, AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex)
{
	if (OtherActor)
	{
		if (Player)
		{
			if (WBP_SCNotify->WidgetAnims.Contains(FName("Intro")))
			{
				...
				if (WidgetAnim)
				{
					...
					GetWorldTimerManager().SetTimer(DelayHandle, this, &ASCSkillElectroSpark::WidgetAnimEnd, WidgetAnim->GetEndTime());
				}
			}
		}
	}
...
void ASCSkillElectroSpark::WidgetAnimEnd()
{
	WBP_SCNotify->RemoveFromParent();
	Player->bCanInteract = false;
}

void ASCSkillElectroSpark::ObjectInteractedWith()
{
	if (Player && !Player->bHasBMagicElectroSpark)
	{
		Player->bHasBMagicElectroSpark = true;
		Player->SkillAcquired = E_Skills::ES_ElectroSpark;
		PlayAcquiredSkillFX();
